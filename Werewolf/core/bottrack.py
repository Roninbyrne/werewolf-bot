import logging from Werewolf import app from pyrogram.types import Chat, ChatMemberUpdated, Message from pyrogram.enums import ChatMemberStatus from pyrogram import filters from motor.motor_asyncio import AsyncIOMotorClient from config import MONGO_DB_URI, OWNER_ID

logging.basicConfig(level=logging.INFO) logger = logging.getLogger(name)

mongo_client = AsyncIOMotorClient(MONGO_DB_URI) db = mongo_client["store"] group_log_db = db["group_logs"] group_members_db = db["group_members"]

@app.on_chat_member_updated() async def handle_bot_status_change(client, update: ChatMemberUpdated): try: if not update.new_chat_member or not update.new_chat_member.user: return
...
async def verify_all_groups_from_db(client): me = await client.get_me() updated_groups = []
...
@app.on_message(filters.command("verifygroups") & filters.user(OWNER_ID)) async def verify_groups_command(client, message: Message): try: logger.info(f"Manual verify triggered by {message.from_user.id}") updated_groups = await verify_all_groups_from_db(client)
