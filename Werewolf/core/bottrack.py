...
async def handle_bot_status_change(client, update: ChatMemberUpdated):
    try:
        ...
        if old_status in (ChatMemberStatus.ADMINISTRATOR, ChatMemberStatus.OWNER) and new_status == ChatMemberStatus.MEMBER:    
            await group_log_db.delete_one({"_id": chat.id})    
            await group_members_db.delete_many({"group_id": chat.id})    
            logger.info(f"❌ Bot was demoted in group {chat.id} — deleted from DB.")    
            try:    
                demote_text = (
                    f"🚪 Bot was demoted to member:\n"
                    f"📛 {chat.title}\n"
                    f"🆔 `{chat.id}`"
                )    
                await client.send_message(LOGGER_ID, demote_text)    
            except Exception as e:    
                logger.warning(f"Failed to send demotion log to LOGGER_ID: {e}")    
            return    
